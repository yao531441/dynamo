...
func DiscoverGPUHardware(
	...
	if metricsDiscovery != nil {
		...
		if filterSKU == nvidiacomv1beta1.GPUSKUTypeB60 {
			if metricsDiscovery.IntelScraper != nil {
				info, err = metricsDiscovery.DiscoverGPUsFromIntelXPUFiltered(ctx, k8sClient, cache, filterSKU)
			} else {
				err = fmt.Errorf("metrics discovery is not configured")
			}
		} else {
			if metricsDiscovery.Scraper != nil {
				info, err = metricsDiscovery.DiscoverGPUsFromDCGMFiltered(ctx, k8sClient, cache, filterSKU)
			} else {
				err = fmt.Errorf("metrics discovery is not configured")
			}
		}
