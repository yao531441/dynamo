...
env "$DYN_VISIBLE_DEVICES_D0" python -m dynamo.vllm --model $MODEL --kv-transfer-config "{\"kv_connector\":\"NixlConnector\",\"kv_role\":\"kv_both\",\"kv_buffer_device\":\"${NIXL_BUFFER_DEVICE}\"}" &
...
env "$DYN_VISIBLE_DEVICES_P0" \
  ...
  --model $MODEL \
  --is-prefill-worker \
  --kv-transfer-config "{\"kv_connector\":\"PdConnector\",\"kv_role\":\"kv_both\",\"kv_buffer_device\":\"${NIXL_BUFFER_DEVICE}\",\"kv_connector_extra_config\":{\"connectors\":[{\"kv_connector\":\"FlexKVConnectorV1\",\"kv_role\":\"kv_both\"},{\"kv_connector\":\"NixlConnector\",\"kv_role\":\"kv_both\"}]},\"kv_connector_module_path\":\"kvbm.vllm_integration.connector\"}" \
